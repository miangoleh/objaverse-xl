...
def render_object(
    ...
    for i in range(num_renders):
        ...
        intrinsics = get_calibration_matrix_K_from_blender(camera.data)

        rt_matrix_path = os.path.join(output_dir, f"{i:03d}.npz")
        np.savez(rt_matrix_path, rt_matrix=rt_matrix, cv_matrix=intrinsics)
